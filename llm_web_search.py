...
def search_duckduckgo(
    query: str,
    max_results: int,
    instant_answers: bool = True,
    regular_search_queries: bool = True,
    get_website_content: bool = False,
) -> list[dict]:
    query = query.strip("\"'")
    with DDGS() as ddgs:
        if instant_answers:
        ...
        if answer_list:
            ...
            for result in ddgs.text(
                query, region="wt-wt", safesearch="moderate", timelimit=None, max_results=max_results
            ):
        ...
        if instant_answers:
            ...
            if answer_list:
                ...
                instant_answer_doc = Document(
                    page_content=answer_dict["text"], metadata={"source": answer_dict["url"]}
                )
        ...
        if instant_answers:
            for answer in answers:
                answer_document = Document(
                    page_content=f"Title: {query}\n{answer}", metadata={"source": "SearXNG instant answer"}
                )
